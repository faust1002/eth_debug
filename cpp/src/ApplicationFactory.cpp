...
std::unique_ptr<Application> createApplication(const IniFile& p_iniFile)
{
    ...
    {
        l_logger = std::make_shared<FileLogger>(LogLevel::DEBUG, std::move(l_formatter),
                p_iniFile.getOption("application", "logfile"));
    }
    else
    {
        l_logger = std::make_shared<ConsoleLogger>(LogLevel::DEBUG, std::move(l_formatter), std::cout);
    }
